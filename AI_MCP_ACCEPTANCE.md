...
**P1**–**P30** 已在打包编辑器上按 ID 实跑。**P31** 是需求里已写、合同还没编号的 camera 与 atlas create 缺参。关编辑器的磁盘 / live 路径（L3-05）是其中一条，不是整表的前提。
...
P1–P31 必须实跑；漏跑 = 失败。禁止把没跑的条目标成通过。仅 L5-07（可选截屏）允许 `SKIP`。
...
优先级：**P0** = 签字必须过。**P1**–**P30** = 已在打包编辑器上跑过。**P31** = 本轮按需求补的编号；失败与 P0 一样要修，不得从合同删掉。
...
| `camera_manage` | L6 | L6-23；P31：L6-36、L6-37 | cube 上有 camera；缺 path / collection+id 为 MISSING_PARAM |
| `tilemap_manage` / `gui_manage` / `input_binding_manage` | L6 | L6-03–L6-05；P29：L6-31、L6-32；P30：L6-34、L6-35 | 读回 tile / text / jump；缺 property / tile / id / x,y 为 MISSING_PARAM |
