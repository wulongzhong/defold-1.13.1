...
**P1**–**P33** 已在打包编辑器上按 ID 实跑。**P34** 是需求里已写、合同还没编号的 input 缺 input，以及 material / particlefx create 缺 path。关编辑器的磁盘 / live 路径（L3-05）是其中一条，不是整表的前提。
...
P1–P34 必须实跑；漏跑 = 失败。禁止把没跑的条目标成通过。仅 L5-07（可选截屏）允许 `SKIP`。
...
优先级：**P0** = 签字必须过。**P1**–**P33** = 已在打包编辑器上跑过。**P34** = 本轮按需求补的编号；失败与 P0 一样要修，不得从合同删掉。
...
| `atlas_manage` … `appmanifest_manage` | L6 | L6-01–L6-22；P10：L6-27；P29：L6-30；P30：L6-33；P31：L6-38；P34：L6-46、L6-47 | 文件 + get + list；假 op 为 UNKNOWN_OP；缺 property / id / path 为 MISSING_PARAM |
