...
2. **观察以结构化快照为主。** 一次观察**始终**把完整 scene_graph 写成文件，MCP 回包默认只有：快照文件句柄、摘要（节点数 / 根 id / 类型直方图）、issues、目标是否还活着。完整树不进上下文。要某一节点或子树，用查询工具打文件。截屏是可选附件，不是 observe 的默认产物，也不是主环必做步骤。连打观察不得撑爆磁盘或回包：快照目录最多保留 **8** 份带 id 的 JSON；默认回包没有 `scene_graph` 且单次信封 ≤ **48 KB**。同一 live 上随机连打也必须守这些上限，WorkingSet 不得爆炸。
